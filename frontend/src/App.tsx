...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-200 flex items-center justify-center px-2">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 flex flex-col items-center">
        <div className="w-full flex justify-end mb-2">
          {user ? (
            <button onClick={handleLogout} className="text-orange-600 font-bold hover:underline">Logout</button>
          ) : (
            <>
              <button onClick={() => setAuthOpen("login")}
                className="text-orange-600 font-bold hover:underline mr-3">Login</button>
              <button onClick={() => setAuthOpen("register")}
                className="text-orange-600 font-bold hover:underline">Register</button>
            </>
          )}
        </div>
